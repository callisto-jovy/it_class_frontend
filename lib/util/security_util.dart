...
class Salt {
  ...
  static String generateAsBase64String(int length) {
    var encoder = const Base64Encoder();
...
class PBKDF2 {
  ...
  PBKDF2({required Hash hashAlgorithm}) {
  ...
  List<int> generateKey(String password, Uint8List salt, int rounds, int keyLength) {
    ...
    var inputBuffer = ByteData(salt.length + 4)
  ...
  String generateBase64Key(String password, Uint8List salt, int rounds, int keyLength) {
    var converter = const Base64Encoder();
